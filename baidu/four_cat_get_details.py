...
#    conn=MySQLdb.connect(host='localhost',user='root',passwd='',db='mafengwo',port=3306,charset='utf8')
    ...
    cur=conn.cursor()
    cur.execute('select four_cat_url,id from baidu_poi where four_cat_url is not null and three_cat_url is not null and addr is null  and total_pingjia is null  order by id')
    result = cur.fetchall()
    print len(result)
    for k,cat_name in enumerate(result[313:]):
...
##                    sql = 'insert into baidu_poi(four_cat_url,three_cat_url,three_cat_rank,three_cat_name,four_cat_name,four_cat_rank,poi_pic,poi_lightspot_title,poi_lightspot_desc,addr,open_time_desc,price_desc,simple_desc,more_desc,recommend_visit_time,total_pingjia) \
##                    values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)'
#                    sql = 'update baidu_poi set jingdian_gonglue=%s where '
...
#        else:
#            need_spider = 1
#            sql = 'insert into baidu_poi(four_cat_url,three_cat_url,three_cat_rank,three_cat_name,four_cat_name,four_cat_rank,addr,open_time_desc,price_desc,simple_desc,more_desc,recommend_visit_time,need_spider,total_pingjia) \
