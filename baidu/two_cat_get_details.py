...
def get_poi_details():
    ...
    cur=conn.cursor()
    cur.execute('select two_cat_url,one_cat_url,one_cat_rank,one_cat_name,two_cat_name,two_cat_rank from baidu_poi where two_cat_url is not null and one_cat_url is not null and addr is null and total_pingjia is null  order by id')
    result = cur.fetchall()
    print len(result)
    for k,cat_name in enumerate(result[14772:]):
        ...
        if json_data and 'high_light_album' in json_data['data']:
            if 'pic_list' in json_data['data']['high_light_album'] and len(json_data['data']['high_light_album']['pic_list']) > 1:
                ...
                for pl in json_data['data']['high_light_album']['pic_list']:
                    ...
                    sql = 'insert into baidu_poi(two_cat_url,one_cat_url,one_cat_rank,one_cat_name,two_cat_name,two_cat_rank,poi_pic,poi_lightspot_title,poi_lightspot_desc,addr,open_time_desc,price_desc,simple_desc,more_desc,recommend_visit_time,total_pingjia) \
            ...
            need_spider = 1
            sql = 'insert into baidu_poi(two_cat_url,one_cat_url,one_cat_rank,one_cat_name,two_cat_name,two_cat_rank,addr,open_time_desc,price_desc,simple_desc,more_desc,recommend_visit_time,need_spider,total_pingjia) \
