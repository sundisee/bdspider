...
def get_city_details():
    ...
    cur=conn.cursor()
    cur.execute('select province_url,province_name,one_cat_url,one_cat_rank,one_cat_name from baidu_poi where province_url is not null and one_cat_url is not null and two_cat_url is null and addr is null  and total_pingjia is null order by id')
    result = cur.fetchall()
    print result
    for k,one_cat_name in enumerate(result[4196:]):
        ajax_url = jingdian_url%(one_cat_name[2])
        print one_cat_name[2],ajax_url
        try:
